...
export type BuzzCardState = 'TagLost' | 'NotABuzzCard' | 'InvalidBuzzCardData' | 'UnknownNfcError' | 'Ready' | 'Processing' | 'BadInternet';
...
const BuzzCardPrompt: React.FC<BuzzCardPromptProps> = ({ 
        ...
        <Modal animationType="fade" transparent={true}>
          <View style={styles.viewContainer}>
            <View style={styles.modalView}>
              ...
              <TextInput style={styles.modalInput} placeholder="90..." 
              maxLength={9} onChangeText={newGtid => setGtid(newGtid)}></TextInput>
              <View style={styles.modalButtonContainer}>
                <View style={styles.modalButton}>
                  <Button onPress={() => processGtid(gtid)} color={currentTheme.primary} title="Enter" />
                </View>
  ...
  return (
    <View style={styles.container}>
      {scanIos && <NfcScanModal scanning={true} appCmd={selectApp} readCmd={readFile} modalText="Tap a BuzzCard" callback={handleNfcResult}></NfcScanModal>}
      ...
      <View>
        {
          ...
          <ActionPrompt icon="contactless" color={currentTheme.error} title="Card read error" 
          subtitle="Hold the BuzzCard up to your phone for a few seconds"
          subtitle2="BuzzCard removed too quickly"></ActionPrompt> :
          ...
          subtitle2="We only support BuzzCards 😉"></ActionPrompt> :

          buzzCardState === 'InvalidBuzzCardData' ?
          <ActionPrompt icon="contactless" color={currentTheme.error} title="Card read error" 
          subtitle="If you are tapping a BuzzCard, please hold it against your phone longer, or reach out in #it-helpdesk for assistance"
          subtitle2="Unexpected BuzzCard data format"></ActionPrompt> :

          buzzCardState === 'BadInternet' ?
          <ActionPrompt icon="contactless" color={currentTheme.error} title="Card read error" 
          subtitle="The last tap was successful, but we couldn't save the data."
          subtitle2="Check your internet connection and try again."></ActionPrompt> :
          ...
          subtitle2="Unknown NFC read error"></ActionPrompt> :
        ...
        <View style={styles.modalButtonContainer}>
          ...
          <View style={styles.modalButton}>
            <Button onPress={() => {setScanIos(true)}} color={currentTheme.primary} title="Scan Card" />
          </View>
          }
          <View style={styles.modalButton}>
            <Button onPress={() => {setEnterGTIDManually(true)}} title="Enter GTID manually" />
          </View>
        </View>
      </View>
